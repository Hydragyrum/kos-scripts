...
FUNCTION OBT_RELATIVE_LAN {
	...
	LOG tgt TO mnv_4.
	LOCAL argp IS SHIP:ORBIT:ARGUMENTOFPERIAPSIS.
	LOCAL r IS tgt:POSITION - SHIP:BODY:POSITION.
	LOCAL v IS tgt:VELOCITY:ORBIT.
	LOCAL h IS VCRS(v,r).
	LOCAL k IS VCRS(SHIP:VELOCITY:ORBIT, SHIP:BODY:POSITION).
	LOCAL n IS VCRS(h,k):NORMALIZED.
	IF n:Z>=0 {
		RETURN ARCCOS(n:X) - argp.
	} ELSE {
		RETURN 360-ARCCOS(n:X) - argp.
	}
}
FUNCTION OBT_TIME_TO_CLOSEST_NODE {
	...
	LOG O TO mnv_4.
	RETURN OBT_MEAN_TIME_FROM_ANGLE(OBT_LNG_TO_DEGREES(OBT_TRUE_TO_MEAN_ANOMALY(O, SHIP:ORBIT:ECCENTRICITY) - OBT_MEAN_ANOMALY(FALSE))).
}
...
FUNCTION OBT_TRUE_TO_MEAN_ANOMALY {
	PARAMETER ta. PARAMETER ecc.
	LOCAL ea IS OBT_TRUE_TO_ECC_ANOMALY(ta, ecc).
	RETURN OBT_ECC_TO_MEAN_ANOMALY(ea, ecc).
}
...
FUNCTION OBT_VELOCITY {
	PARAMETER ra. PARAMETER pe. PARAMETER ap. PARAMETER b.
	LOCAL a IS ((pe+ap)/2)+b:RADIUS.
	return SQRT(b:MU*((2/(ra+b:RADIUS))-(1/a))).
}
FUNCTION OBT_RADIUS_AT_TRUE_ANOMALY{
	PARAMETER ta. PARAMETER a. PARAMETER ecc.
	RETURN a * ((1-ecc*ecc)/1+(ecc*cos(ta))).
}
